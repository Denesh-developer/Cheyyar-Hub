  ...
  async function handleAuth(e) {
    ...
    try {
      ...
      if (
        ...
        // Create the auth account FIRST. Firestore rules require
        // isSignedIn() to read /users/*, so the username-uniqueness
        // check below must run AFTER auth, not before — running it
        // before auth is what caused "Missing or insufficient
        // permissions" on signup.
        const cred =
        ...
        // Force a fresh ID token so Firestore rules see a valid
        // request.auth immediately (avoids a race where the token
        ...
        try {
          ...
          // Roll back the auth account so we don't leave an orphaned
          // login with no profile doc (e.g. username was taken).
