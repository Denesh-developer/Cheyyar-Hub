...
async function sendEmailOtp(email, otp, env) {
  ...
  if (!response.ok) {
    throw new Error(data?.message || data?.error || 'Resend failed to send the email.');
  }
  return data;
}

async function handleSendOtp(request, env) {
  if (!env.RESEND_API_KEY) throw new Error('RESEND_API_KEY is not configured on the Worker.');
  if (!env.OTP_STORE) throw new Error('OTP_STORE is not configured.');
  ...
  const key = `otp:${email}`;
  const existing = await env.OTP_STORE.get(key, { type: 'json' });
  if (existing?.sentAt) {
...
async function handleVerifyOtp(request, env) {
  if (!env.FIREBASE_SERVICE_ACCOUNT) throw new Error('FIREBASE_SERVICE_ACCOUNT is not configured on the Worker.');
  if (!env.FIREBASE_PROJECT_ID || env.FIREBASE_PROJECT_ID.startsWith('REPLACE_')) {
    throw new Error('FIREBASE_PROJECT_ID is not configured correctly.');
  }
  ...
  const key = `otp:${email}`;
  const record = await env.OTP_STORE.get(key, { type: 'json' });
  if (!record) {
  ...
  const customToken = await createFirebaseCustomToken(email, serviceAccount, env.FIREBASE_PROJECT_ID);
